...
def build_train_and_save_model(params):
    input_coeffs_file = "dwt_data/train_input.csv"
    output_coeffs_file = "dwt_data/train_output.csv"
    ...
    input_torch = (torch.tensor(input_df.values[:, 1:no_of_coeffs*input_dim+1], dtype=torch.float32)).reshape(-1, no_of_coeffs, input_dim)  #140*944*9

    output_df = pd.read_csv(output_coeffs_file)
    expected_output = torch.tensor(output_df.values[:, 1:no_of_coeffs+1], dtype=torch.float) #140 * 944
