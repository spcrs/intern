...
def plot_result(params,fold,split):
  
  #prepare predicted data
  df = pd.read_csv(f"result/result{split}/predicted_output_fold{fold}.csv")
  predicted = df.values[:,1:].tolist()

  expected = pd.read_csv(f"expected/expected{split}/expected_output_fold{fold}.csv").values[:,1:-1]

  file_detail = pd.read_csv(f"expected/expected{split}/expected_output_fold{fold}.csv").values[:,-1]
  ...
  for i in range(len(expected)):
    ...
    plt.savefig(f'graph/output{split}/preds_folds{fold}{i}.png')



  no_of_test_files = len(os.listdir(f"data/train{split}/"))/params.folds
  ...
  filename = f"result/Metrics{split}.csv"
  if(fold == 1):
